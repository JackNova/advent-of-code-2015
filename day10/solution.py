...
# and using that reading as the next sequence. For example, 211 is read as "one two, two ones",
...
def read_aloud(number):
	return ''.join( str(len(list(g))) + k for k, g in itertools.groupby(number))

for n in xrange(0,40):
	input = read_aloud(input)
	print n
	print len(input)