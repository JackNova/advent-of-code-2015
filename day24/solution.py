...
# such that the fewest possible are in the first group,
# you need to choose the way where the first group has the smallest quantum entanglement
# to reduce the chance of any "complications".
...
# Only consider quantum entanglement if the first group has the fewest possible number of packages in it
# and all groups weigh the same amount.

# For example, suppose you have ten packages with weights 1 through 5 and 7 through 11.
test_packages = range(1, 6) + range(7, 12)
assert len(test_packages) == 10

def balanced_groups(packages, sort_key=len):
	for passenger_compartment, left, right in algorithm_u(test_packages, 3):
		if sum(passenger_compartment) == sum(left) == sum(right):
			yield tuple(sorted([passenger_compartment, left, right], key=sort_key))

def quantum_entanglements(pkgs):
	return reduce(operator.mul, pkgs, 1)

def solve(packages):
	groups = list(balanced_groups(packages))
	min_passenger_compartment_packages = len(min(groups, key=lambda (pc, l, r) : len(pc))[0])
	print 'the minimum number of packages in passenger compartment is %s' % min_passenger_compartment_packages
	optimal_solutions = [(pc, l, r) for (pc, l, r) in groups if len(pc) == min_passenger_compartment_packages]
	print 'there are %s possible arrangements with %s packages in passenger compartment' % (len(optimal_solutions), min_passenger_compartment_packages)
	optimal_quantum_entanglements = min(optimal_solutions, key=lambda (pc, l, r) :quantum_entanglements)
	print 'the optimal solution have %s quantum entanglement' % quantum_entanglements(optimal_quantum_entanglements[0])
	return optimal_quantum_entanglements
...
assert quantum_entanglements( solve(test_packages)[0] ) == 99
